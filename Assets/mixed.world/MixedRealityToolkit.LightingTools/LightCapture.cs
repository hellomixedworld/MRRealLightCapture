...
namespace Microsoft.MixedReality.Toolkit.LightingTools
{
	public class LightCapture : MonoBehaviour
	{
        ...
        [Tooltip("Should we use the MediaProjection API to capture the Camera?")]
        [SerializeField] private bool useMediaProjection = true;
        [Tooltip("If using the MediaProjection API, fill in the texture that you render to here.")]
        [SerializeField] private RawImage mediaprojectionTexture;
		...
		private void Awake ()
		{
			...
			{
                captureCamera = new CameraCaptureMediaProjection(mediaprojectionTexture);
            }
            else
			{
